 ...
 */
export function convertExpressPathToOpenAPI(expressPath: string): string {
  ...
  return expressPath.replace(
    paramRegex,
    (
      match, // Вся совпавшая строка, например, ":id(\\d+)?"
      paramName, // Захваченное имя параметра, например, "id"
    ) => `{${paramName}}`,
  );
}
