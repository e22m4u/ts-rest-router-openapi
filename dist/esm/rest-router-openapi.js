...
export class RestRouterOpenAPI extends Service {
    ...
    addParameterToOAOperation(oaOperation, paramName, oaLocation, paramSchema) {
        ...
        if (oaLocation === OAParameterLocation.PATH)
    ...
    genOpenAPIDocument(doc) {
        if (!this.hasService(RestRouter))
            throw new Errorf('A RestRouter instance must be registered ' +
        ...
        for (const cls of controllers) {
            ...
            if (!controllerMd)
                throw new Errorf('Controller class %s does not have metadata.', cls.name);
            // формирование тега
            ...
            for (const [actionName, actionMd] of actionsMd.entries()) {
                ...
                const operationPath = (actionMd.path ?? '')
                    .replace(/(^\/+|\/+$)/, '')
                    .replace(/\/+/g, '/');
                const fullOperationPath = `/${tagPath}/${operationPath}`
                    .replace(/\/+$/, '')
                    .replace(/\/+/g, '/') || '/';
                ...
                const requestDataMdMap = RequestDataReflector.getMetadata(cls, actionName);
                ...
                for (const requestDataMd of requestDataMds) {
                    ...
                    if (REQUEST_DATA_SOURCE_TO_OPENAPI_LOCATION_MAP.get(requestDataMd.source) &&
                        ...
                        const paramSchema = (requestDataMd.schema &&
                            typeof requestDataMd.schema === 'object' &&
                            requestDataMd.schema.properties &&
                            typeof requestDataMd.schema.properties === 'object' &&
                            requestDataMd.schema.properties[requestDataMd.property] &&
                            typeof requestDataMd.schema.properties[requestDataMd.property] === 'object' &&
                            requestDataMd.schema.properties[requestDataMd.property]) ||
                            undefined;
                    ...
                    // PATH, QUERY, HEADER, COOKIE
                    // (пропускаются типы отличные от OBJECT)
                    else if (REQUEST_DATA_SOURCE_TO_OPENAPI_LOCATION_MAP.get(requestDataMd.source) &&
                        requestDataMd.schema &&
                        requestDataMd.schema.type === DataType.OBJECT &&
                        requestDataMd.schema.properties &&
                        typeof requestDataMd.schema.properties === 'object' &&
                ...
                if (responseBodyMd && responseBodyMd.schema) {
                    ...
                    // формирование схемы данных, используя STRING
                    // как значение по умолчанию (вместо ANY)
                    const oaSchema = dataSchemaToOASchemaObject(responseBodyMd.schema, OADataType.STRING);
